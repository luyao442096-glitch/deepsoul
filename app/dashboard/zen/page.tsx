...
export default function MusicPage() {
  ...
  const playSound = (index: number) => {
    ...
    if (!song.unlocked) {
      return; // Don't play locked songs
    }
  ...
  const skipPrevious = () => {
    if (!isSubscribed && currentSong > 0) {
      // If not subscribed, can only play the first song
      playSound(0);
    } else {
  ...
  return (
    <div className="min-h-screen w-full bg-[#F8F5F0] text-[#333333] flex flex-col">
      ...
      <div className="absolute top-6 left-6 z-50 flex items-center gap-2 text-[#666666] text-sm font-serif">
        ...
        <span>/</span>
        <span className="text-[#333333]">Zen</span>
      </div>
      {/* Header */}
      ...
      <main className="flex-1 max-w-4xl mx-auto px-4 py-8">
        ...
        >
          ...
          <p className="text-[#666666] max-w-2xl mx-auto">
            ...
            {!isSubscribed && ' Subscribe to unlock all tracks.'}
          </p>
        </motion.div>
